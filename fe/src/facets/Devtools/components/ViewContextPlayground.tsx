...
const ViewContextPlayground: React.FC<IndexedLayoutChildProps> = () => {
  ...
  return (
    <Stack gap={2} alignItems="flex-start">
      ...
      <Checkbox
        label="Place sidebar on the right"
        onChange={(e) => {
          configureView({
            sidebarPlacement: e.target.checked ? 'right' : 'left',
          });
        }}
      />

      <Checkbox
        label="Show pinned widget"
        onChange={(e) => {
          configureWidgets({
            slot: 0,
            component: e.target.checked ? <DemoLoadingWidget /> : null,
          });
        }}
      />
      ...
      <Button
        variant="solid"
        onClick={() =>
          openDrawer(<Typography m={2}>Drawer contents</Typography>, 'Drawer')
        }>
        Open drawer
      </Button>
    </Stack>
