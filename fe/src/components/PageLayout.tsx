...
const Content: React.FC<PropsWithChildren<ContentLayoutProps>> = ({
  children,
  noTitle,
}) => {
...
const Centered: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  return (
    <Box
      sx={{
        ...paddingSxProps(view?.sidebarPlacement || 'left'),
        minHeight: '100dvh',
        display: 'flex',
        alignItems: 'center',
  ...
  const tabs = useMemo(() => {
    ...
    if (result?.length !== childrenArray.length) {
      developmentLogger.warn(
        'Indexed page layout should only contain IndexedLayoutChildProps elements.'
      );
    }
      ...
      <TabList underlinePlacement={sidebarPlacement} sx={{ py: 2 }}>
        {tabs?.map((tab, index) => (
          <Tab
            key={`tab-${index}`}
            value={tab.key}
            indicatorPlacement={sidebarPlacement}>
            {tab.caption || tab.key}
          </Tab>
        ))}
      </TabList>
