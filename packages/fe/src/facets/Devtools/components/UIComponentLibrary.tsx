...
const editableTypography: UIComponentCardProps<
  EditableTypographyProps & { rejectChanges?: boolean }
> = {
    ...
    <>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Checkbox
          ...
          onChange={(e) =>
            dispatch((prev) => ({
              ...prev,
              variant: e.target.checked ? 'soft' : 'plain',
            }))
          }
        />
        <Checkbox
          ...
          onChange={(e) =>
            dispatch((prev) => ({ ...prev, disabled: e.target.checked }))
          }
        />
        <Checkbox
          size="sm"
          ...
          onChange={(e) =>
            dispatch((prev) => ({
              ...prev,
              rejectChanges: e.target.checked,
              color: e.target.checked ? 'danger' : 'primary',
            }))
          }
        />
      </Box>
      <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
        ...
        <Radio
          ...
          checked={
            state.level === 'title-lg' ||
            state.level === 'title-md' ||
            state.level === 'title-sm'
          }
...
const pieProgress: UIComponentCardProps<PieProgressProps> = {
  ...
  displayPanel: (state) => (
    <AspectRatio
      ratio={1}
      color="primary"
      sx={{ width: '120px', borderRadius: '50%' }}>
      <PieProgress {...state} />
    </AspectRatio>
  ),
  controlPanel: (state, dispatch) => (
    <>
      <FormControl size="sm">
        ...
        <Input
          ...
          onChange={(e) =>
            dispatch((prev) => ({ ...prev, value: parseInt(e.target.value) }))
          }
        />
      </FormControl>
      <Checkbox
        ...
        onChange={(e) =>
          dispatch((prev) => ({ ...prev, zeroIndicator: e.target.checked }))
        }
      />
      <Checkbox
        size="sm"
        ...
        onChange={(e) =>
          dispatch((prev) => ({ ...prev, filled: e.target.checked }))
        }
      />
    </>
  ),
